...
where
    F: Fn(Pid, TracerMessage) -> ampsc::Send<'a, Message> + std::marker::Send + std::marker::Sync,
{
    ...
    match sci.data {
        ...
        PtraceSyscallInfoData::Entry(entry) => {
            ...
            match syscall {
                ...
                Syscall::Execve(execve) => {
                    ...
                    send(pid, TracerMessage::SubchildSpawning(
        ...
        PtraceSyscallInfoData::Exit(exit) => match last_syscall {
            ...
            Some(Syscall::Write(write)) => {
                ...
                send(pid, TracerMessage::ShellOutput(write.fd, text))
                    .await
                    .expect(FMPSCSEND);
                match write.fd {
    ...
    F: Fn(Pid, TracerMessage) -> ampsc::Send<'a, Message> + std::marker::Send + std::marker::Sync,
{
    ...
    match w {
        wait::WaitStatus::Exited(_, retv) => {
            send(pid, TracerMessage::ChildExited(TerminationStatus::Exited(retv)))
                .await
                .expect(FMPSCSEND);
            TraceRetv::Exit
        }
        wait::WaitStatus::Signaled(_, signal, _) => {
            send(pid, TracerMessage::ChildExited(TerminationStatus::Killed(
                signal,
            )))
            .await
            .expect(FMPSCSEND);
